...
### Statistics
All statistics are sorted by date (from oldest to newest). This is *required*
for the import to work correctly, so make sure the order is correct if you're
generating JSON files yourself.
...
references `locations.country` or `locations.country || '-' || locations.region`.
