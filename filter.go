...
func (f *Filter) Insert(ctx context.Context, paths []PathID) error {
	...
	err := zdb.TX(ctx, func(ctx context.Context) error {
		err := zdb.Exec(ctx,
			`insert into filters (filter_id, site_id, query, matches, invert, created_at, last_used_at) values (?, ?, ?, ?, ?, ?, ?)`,
			f.FilterID, MustGetSite(ctx).ID, f.Query, f.Matches, f.Invert, f.CreatedAt, f.LastUsedAt)
		if err != nil {
			return err
		}
