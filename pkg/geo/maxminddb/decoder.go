...
func (d *decoder) decodePath(offset uint, path []any, result reflect.Value) error {
PATH:
	for i, v := range path {
		var (
			typeNum dataType
...
func (d *decoder) decodeFromType(dtype dataType, size uint, offset uint, result reflect.Value, depth int) (uint, error) {
...
func (d *decoder) decodeFromTypeToDeserializer(dtype dataType, size uint, offset uint, dser deserializer, depth int) (uint, error) {
	// For these types, size has a special meaning
	switch dtype {
	case _Bool:
